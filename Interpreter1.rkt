...
(define apply-env
  (lambda (var-name env)
    (cond
      ((null? env) #f)
      ((has-binding? var-name (peek-scope env)) (apply-scope var-name (peek-scope env)))
  ...
  (lambda (lcexp env)
    (cond
      ...
      ((do-in-order-exp? lcexp) (letrec ((loe (do-in-order-exp->list-of-expressions lcexp))
                                         (processList (lambda (loe currScope)
                                           (cond
                                             ...
                                             (else (cons (apply-expression (car loe) currScope)
      ...
      ((app-exp? lcexp) (let* ((the-lambda (app-exp->lambda-exp lcexp))
                              ...
                              (the-new-scope (extend-env the-lambda-param-name the-parameter-value env)))
